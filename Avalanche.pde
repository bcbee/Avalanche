...
void setVars() {
  ...
  iceX = int(emptyArray);
  iceY = emptyArray;
  iceState = int(emptyArray);
  iceTimer = int(emptyArray);
  //Player x,y,score
...
void drawMenus() {
  ...
  if (menu == 2) {
    ...
    text(int(player[2]), halfWidth, halfHeight-50);
  ...
  if (menu > 1) {
    if (mouseX > halfWidth-125) {
      if (mouseX < halfWidth+125) {
        ...
        if (mouseY < halfHeight - 125) {
          if (mouseY > halfHeight - 175) {
            url = "http://dbztech.com/Avalanche/redirect.php?input="+int(player[2]);
            link(url);
          }
        }
      }
